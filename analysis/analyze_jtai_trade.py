"""
JTAI Trade Analysis
Analyze why the JTAI trade was exited prematurely and identify fixes
"""
import sys
import os
...
def get_jtai_trades():
    """Get all JTAI trades from database"""
    db = TradingDatabase()
    trades = db.get_trades_by_ticker('JTAI')
    positions = db.get_active_positions()
    jtai_positions = [p for p in positions if p.get('ticker') == 'JTAI']
    
    logger.info(f"Found {len(trades)} JTAI trade(s) in database")
    logger.info(f"Found {len(jtai_positions)} active JTAI position(s)")
    
    return trades, jtai_positions


def download_jtai_data():
    """Download comprehensive data for JTAI"""
    api = WebullDataAPI()
    
    logger.info("Downloading JTAI data from Webull API...")
    
    try:
        # Get multiple timeframes
        data_1min = api.get_1min_data('JTAI', minutes=1200)  # Max available
        data_5min = api.get_5min_data('JTAI', periods=500)    # ~41 days
...
def analyze_trade_exit(data_1min, data_5min, trade):
    ...
    logger.info(f"\n{'='*80}")
    logger.info(f"ANALYZING JTAI TRADE")
    logger.info(f"{'='*80}")
...
def main():
    ...
    logger.info("="*80)
    logger.info("JTAI TRADE ANALYSIS")
    logger.info("="*80)
    
    try:
        # Get trades from database
        trades, positions = get_jtai_trades()
        
        if not trades:
            logger.warning("No JTAI trades found in database")
            if positions:
                logger.info(f"Found {len(positions)} active JTAI position(s)")
            return
        
        # Download data
        data_1min, data_5min = download_jtai_data()
        ...
        # Save summary report
        output_file = Path(__file__).parent / 'jtai_analysis_report.txt'
        with open(output_file, 'w') as f:
            f.write("JTAI Trade Analysis Report\n")
            f.write("="*80 + "\n\n")
            for trade in trades:
