"""
UAVS Trade Analysis
Analyze why the UAVS trade was exited prematurely and identify fixes
"""
import sys
import os
...
def get_uavs_trades():
    """Get all UAVS trades from database"""
    db = TradingDatabase()
    trades = db.get_trades_by_ticker('UAVS')
    positions = db.get_active_positions()
    uavs_positions = [p for p in positions if p.get('ticker') == 'UAVS']
    
    logger.info(f"Found {len(trades)} UAVS trade(s) in database")
    logger.info(f"Found {len(uavs_positions)} active UAVS position(s)")
    
    return trades, uavs_positions


def download_uavs_data():
    """Download comprehensive data for UAVS"""
    api = WebullDataAPI()
    
    logger.info("Downloading UAVS data from Webull API...")
    
    try:
        # Get multiple timeframes
        data_1min = api.get_1min_data('UAVS', minutes=1200)  # Max available
        data_5min = api.get_5min_data('UAVS', periods=500)    # ~41 days
...
def analyze_trade_exit(data_1min, data_5min, trade):
    ...
    logger.info(f"\n{'='*80}")
    logger.info(f"ANALYZING UAVS TRADE")
    logger.info(f"{'='*80}")
    ...
    if 'trailing stop' in trade['exit_reason'].lower():
        ...
        trailing_stop_pct = 2.5  # Current setting
...
def rerun_trade_analysis(data_1min, trade):
    ...
    # Find entry signal area
    entry_window = data_1min[
        (data_1min['timestamp'] >= entry_time - timedelta(minutes=30)) &
        (data_1min['timestamp'] <= entry_time + timedelta(minutes=30))
    ]
    
    logger.info(f"\nAnalyzing entry signals around {entry_time}...")
...
def main():
    ...
    logger.info("="*80)
    logger.info("UAVS TRADE ANALYSIS")
    logger.info("="*80)
    
    try:
        # Get trades from database
        trades, positions = get_uavs_trades()
        
        if not trades:
            logger.warning("No UAVS trades found in database")
            if positions:
                logger.info(f"Found {len(positions)} active UAVS position(s)")
            return
        
        # Download data
        data_1min, data_5min = download_uavs_data()
        ...
        # Save summary report
        output_file = Path(__file__).parent / 'uavs_analysis_report.txt'
        with open(output_file, 'w') as f:
            f.write("UAVS Trade Analysis Report\n")
            f.write("="*80 + "\n\n")
            for trade in trades:
