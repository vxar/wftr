"""
Simulate bot trading for WWR focusing on after-hours trading
Downloads data from market close (4 PM) onwards and runs bot simulation
"""
import sys
import os
...
# Stock to simulate
TICKER = 'WWR'
AFTER_HOURS_START_HOUR = 16  # 4 PM (market close)

def download_and_save_data(ticker, api, test_data_dir, start_hour=16):
    """Download 1-minute data from start_hour (after-hours) and save to test_data folder"""
    
    et = pytz.timezone('US/Eastern')
    today = datetime.now(et).date()
    
    logger.info(f"Downloading after-hours data for {ticker} from {start_hour}:00 (4 PM)...")
    
    try:
        # Fetch data (get enough minutes to cover after-hours)
        ...
        # Filter to today and from start_hour (after-hours)
        df_today = df[df['date'] == today].copy()
        df_today['hour'] = df_today['timestamp'].dt.hour
        df_today = df_today[df_today['hour'] >= start_hour].copy()
        
        if df_today.empty:
            logger.warning(f"No after-hours data for {ticker} from {start_hour}:00 today")
            return None
        ...
        filename = f"{ticker}_1min_afterhours_{today.strftime('%Y%m%d')}.csv"
        filepath = os.path.join(test_data_dir, filename)
        df_save.to_csv(filepath, index=False)
        logger.info(f"Saved {len(df_save)} minutes of after-hours data to {filepath}")
...
def simulate_bot_trading(ticker, df_data, trader):
    ...
    logger.info(f"Simulating bot trading for {ticker} (after-hours)...")
...
def main():
    ...
    logger.info(f"\n{'='*80}")
    logger.info(f"Simulating {TICKER} after-hours trading from 4 PM ({today})")
    logger.info(f"{'='*80}")
    
    # Download and save data
    df_data = download_and_save_data(TICKER, api, test_data_dir, AFTER_HOURS_START_HOUR)
    
    if df_data is None or df_data.empty:
        logger.warning(f"Skipping {TICKER} - no after-hours data available")
        return
    ...
    if trades:
        ...
        csv_file = f"analysis/BOT_SIMULATION_{TICKER}_AFTERHOURS_{today.strftime('%Y%m%d')}.csv"
        ...
        logger.info(f"No trades generated for {TICKER} during after-hours")
