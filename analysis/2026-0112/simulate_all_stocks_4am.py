"""
Simulate bot trading for all analyzed stocks from 4 AM today
Runs full bot simulation and generates BOT_SIMULATION CSV files
"""
import sys
import os
...
# Stocks to simulate (from analyze_multiple_stocks_4am.py)
STOCKS_TO_SIMULATE = ['OM', 'EVTV', 'SOGP', 'INBS', 'UP', 'BDSX', 'ATON', 'WWR']
START_HOUR = 4  # 4 AM

def download_and_save_data(ticker, api, test_data_dir, start_hour=4):
    """Download 1-minute data from start_hour and save to test_data folder"""
    
    et = pytz.timezone('US/Eastern')
    today = datetime.now(et).date()
    
    logger.info(f"Downloading full day data for {ticker} from {start_hour}:00 AM...")
    
    try:
        # Fetch data (get enough minutes to cover full day including after-hours)
...
def simulate_bot_trading(ticker, df_data, trader):
    ...
    for idx in range(len(df)):
        ...
        if entry_signal and ticker not in trader.active_positions:
            # IMPROVED: Use trader.enter_position() to get proper ATR-based stop loss
            # This ensures stop loss is calculated based on volatility and entry type
            position = trader.enter_position(entry_signal, df_slice)
            
            # Set shares and entry value for simulation
            shares = 1000  # Fixed shares for simulation
            position.shares = shares
            position.entry_value = shares * position.entry_price
            position.original_shares = shares
            
            logger.info(f"[ENTRY] {ticker} @ ${entry_signal.price:.4f} - {entry_signal.pattern_name} ({entry_signal.confidence*100:.1f}%)")
            logger.info(f"  Entry time: {entry_signal.timestamp.strftime('%H:%M:%S')}")
            logger.info(f"  Stop Loss: ${position.stop_loss:.4f} ({((position.entry_price - position.stop_loss) / position.entry_price) * 100:.2f}%)")
            logger.info(f"  Target: ${position.target_price:.4f} ({((position.target_price - position.entry_price) / position.entry_price) * 100:.2f}%)")
...
def simulate_stock(ticker, api, trader, test_data_dir, today):
    """Simulate a single stock"""
    
    logger.info(f"\n{'='*80}")
    logger.info(f"Simulating {ticker} from 4 AM ({today})")
    logger.info(f"{'='*80}")
    ...
    if trades:
        ...
        csv_file = f"analysis/BOT_SIMULATION_{ticker}_4AM_{today.strftime('%Y%m%d')}.csv"
        ...
        # Create empty CSV file to document that simulation was run
        csv_file = f"analysis/BOT_SIMULATION_{ticker}_4AM_{today.strftime('%Y%m%d')}.csv"
        empty_df = pd.DataFrame(columns=['ticker', 'entry_time', 'exit_time', 'entry_price', 'exit_price', 'shares', 
                                         'entry_value', 'exit_value', 'pnl_dollars', 'pnl_pct', 'entry_pattern', 
                                         'exit_reason', 'confidence', 'hold_time_minutes', 'max_price', 'max_price_pct', 'capture_rate'])
        empty_df.to_csv(csv_file, index=False)
        logger.info(f"Created empty CSV file: {csv_file} (no trades generated)")
        return None
...
def main():
    ...
    logger.info(f"\n{'='*80}")
    logger.info(f"Simulating All Stocks from 4 AM ({today})")
    logger.info(f"Stocks: {', '.join(STOCKS_TO_SIMULATE)}")
    logger.info(f"{'='*80}\n")
    ...
    # Simulate each stock
    for ticker in STOCKS_TO_SIMULATE:
        try:
            # Create new trader instance for each stock to reset state
            trader = RealtimeTrader(
                min_confidence=0.72,
                profit_target_pct=20.0,
                trailing_stop_pct=7.0
            ...
            trades = simulate_stock(ticker, api, trader, test_data_dir, today)
            all_results[ticker] = trades
            
        except Exception as e:
            logger.error(f"Error simulating {ticker}: {e}", exc_info=True)
    ...
    for ticker, trades in all_results.items():
        if trades:
            ...
            total_pnl_all += sum(t['pnl_dollars'] for t in trades)
            total_wins += len([t for t in trades if t['pnl_dollars'] > 0])
            total_losses += len([t for t in trades if t['pnl_dollars'] < 0])
            logger.info(f"{ticker}: {len(trades)} trades, ${sum(t['pnl_dollars'] for t in trades):+.2f} P&L")
        else:
            logger.info(f"{ticker}: No trades")
